...
3  wiring:  the fly's exact wiring did not matter; error in points, two seeds per arm, mean marked

Every number is read from results/results.json (written by scripts/collect_results.py). Palette from viz/figures.py.
Run: uv run --project ~/Projects/ds-lab python viz/article_images.py
"""
...
OUT = ROOT / "viz" / "article"; OUT.mkdir(exist_ok=True)
R = json.load(open(ROOT / "results" / "results.json"))
RES = R["results"]; FX = R["extra"]["frontier"]
SRC = "Source: SuperTruth, Intelligence Is Structure, Not Scale, v1.1 (results.json, 21 Sep 2026), doi:10.5281/zenodo.22865215. Synthetic records only."
...
def cover():
    ...
    fig.text(0.045, 0.86, "A fruit fly's brain beat\nClaude, GPT-5, Grok and Gemini\nat judging health records.", fontsize=44, weight="bold", va="top", linespacing=1.15)
    ...
    fig.text(0.045, 0.105, f"Seen from the front: {meta['n_neurons'] - meta['n_unpositioned']:,} of {meta['n_neurons']:,} nerve cells at their recorded positions, 6,242,118 connections. Synthetic records only; no real patient's data.\n" + FOOTER +
             "\nThe test measures agreement with SuperTruth's own DTI score and ranks no vendor. SuperTruth, Intelligence Is Structure, Not Scale (2026), doi:10.5281/zenodo.22865215. Two of five planned runs; provisional.",
...
def models():
    ...
    fig, ax = plt.subplots(figsize=(12, 7.2)); fig.subplots_adjust(left=0.20, right=0.90, top=0.76, bottom=0.22)
    ...
    ax.set_xlabel("Records placed in the right trust level, out of 100 (same 300 test records)", fontsize=12, color=C["ink2"])
    for sp in ("top", "right", "left"): ax.spines[sp].set_visible(False)
    ax.grid(axis="x", color=C["hairline"], lw=0.8); ax.set_axisbelow(True)
    fig.text(0.03, 0.965, "The fly beat all four models, with or without help", fontsize=20, weight="semibold", va="top")
    fig.text(0.03, 0.895, "Hollow dot: the model was given the published DTI paper. Solid dot: the paper plus 100 scored example records (a second, post-hoc round).\n"
             "Thin grey bars: 95% intervals over the 300 records; gaps under about 6 points are within noise. The blue line is the fly's fixed wiring on the same records.",
             fontsize=12, color=C["ink2"], va="top", linespacing=1.45)
    fig.text(0.03, 0.095, "Right trust level means the same level SuperTruth's own DTI engine assigned; the test does not judge who was right about the records and ranks no vendor.\n"
             "Fly: seed 1, a single point with no interval; bitwise identical on repeat. Models: identical trust level on 67% to 95% of records across three repeats.\n" + SRC,
             fontsize=9.5, color=C["ink2"], va="top", linespacing=1.4)
...
def wiring():
    arms = [("connectome", "The fly's real wiring"), ("shuffled", "The fly's wiring, scrambled"), ("random", "A random web, same size"),
            ("mlp", "Ordinary trained network, same size"), ("linear", "Straight-line fit (two identical runs)")]
    fig, ax = plt.subplots(figsize=(12, 7.2)); fig.subplots_adjust(left=0.30, right=0.95, top=0.78, bottom=0.17)
    ...
    ax.set_xlim(0, 4.2); ax.set_xticks([0, 1, 2, 3, 4]); ax.tick_params(axis="x", labelsize=11)
    ...
    fig.text(0.03, 0.895, "Scrambling the fly's wiring gave the same result; a random web of the same size came within a quarter of a point. All three fixed webs beat an\n"
             "ordinary trained network with the same number of adjustable parts. Small dots: the two runs so far. Bar: their average.",
             fontsize=12, color=C["ink2"], va="top", linespacing=1.45)
    fig.text(0.03, 0.05, "Two of five planned runs; provisional. " + SRC, fontsize=9.5, color=C["ink2"], va="top")

...
<!-- STATUS: v1.0 provisional (2026-09-20). Front matter, plain-language opener, attribution, intended use, COI, footer are SETTLED
     ...
     not produced by a run. Version 1.0 = seed 1 of five for every finished arm; cells with no run say "not in this version"
     (render_results.py --version 1.0); the live spot check and the window-cap parity check are SKIPPED with reason; the dataset DOI
     placeholder in Data and Code Availability is filled by splice_paper.py --dataset-doi once the Zenodo dataset record exists. Version 1.1 adds the BII controls, g2 and seeds 2 to 5.
...
<small>*Provenance: as Table 8a; snapshot 2026-09-20; seeds 1 of 5; evaluated 2026-09-20T22:07:56+00:00.*</small>
...
| Degree-preserving shuffle (b) | 0.0258 | 2.58 | 7,811,162 | 1 of 5, single seed |
| Random graph, matched density (c) | not in this version |  |  | 0 |
| Matched-parameter network (d) | not in this version |  |  | 0 |
| Linear floor, ridge (e) | 0.0724 | 7.24 | 295 (closed form) | 1 of 5, single seed |
| Teacher (f) | 0 | 0 | 0 | by construction |

<small>*Provenance: as Table 8a; snapshot 2026-09-20; seeds 1 of 5; evaluated 2026-09-20T22:07:56+00:00. BII differs: teacher VIGIL commit 1cffd016b7bf + policy digest 5d8db364...8573859, clock frozen at 2026-09-20T00:00:00Z; data teachers/bii_teacher.jsonl (gen_bii.py) sha256 c43bb314823f, run_seed 20260920, 14,000 / 2,000 / 4,000 records (train / validation / test), split sha256 c42920804158, synthetic, zero PHI; coverage TEST split only, 4,000 records, seed 1 of the protocol's five; teacher dimensions excluded as degenerate: none (splits/qa_report.json); feature spec 20a2cd2c28bc; language-model arms not run for this task.*</small>
...
| Degree-preserving shuffle (b) | 0.912 | 0.904 | 0.010 | 1 of 5, single seed |
| Random graph, matched density (c) | not in this version |  |  | 0 |
| Matched-parameter network (d) | not in this version |  |  | 0 |
| Linear floor, ridge (e) | 0.597 | 0.426 | 0.307 | 1 of 5, single seed |
| Teacher (f) | 1.000 | 1.000 | 0 | by construction |

<small>*Provenance: as Table 8a; snapshot 2026-09-20; seeds 1 of 5; evaluated 2026-09-20T22:07:56+00:00. BII rows as Table 8b.*</small>
...
<small>*Provenance: as Table 8a; snapshot 2026-09-20; seeds 1 of 5; evaluated 2026-09-20T22:07:56+00:00. Model-arm settings and prices as the line under Table 9.*</small>
...
| BII | Wiring matters | δ_b = MAE(shuffle) − MAE(connectome) | ≥ 0.015, CI lower > 0, connectome better on every seed | -0.0030 | **INSUFFICIENT SEEDS (1/5)** | misses |
| BII | Wiring matters | δ_c = MAE(ER) − MAE(connectome) | ≥ 0.015, CI lower > 0, connectome better on every seed | not in this version | **NOT IN THIS VERSION** |  |
| BII | Fly beats the floor | MAE(ridge) − MAE(connectome) | > 0, CI lower > 0 | 0.0436 | **INSUFFICIENT SEEDS (1/5)** | meets |
| BII | Fixed graph beats matched network (framing, descriptive) | MAE(MLP) − MAE(connectome) | ≥ 0.015, CI lower > 0, connectome better on every seed (same margin as the wiring rule; not pre-registered) | not in this version | **NOT IN THIS VERSION** |  |
...
| BII | Wiring matters | δ_b = MAE(shuffle) − MAE(connectome) | 1 | no | n/a (one seed) |
| BII | Wiring matters | δ_c = MAE(ER) − MAE(connectome) | not in this version |  |  |
| BII | Fly beats the floor | MAE(ridge) − MAE(connectome) | 1 |  | n/a (one seed) |
| BII | Fixed graph beats matched network (framing, descriptive) | MAE(MLP) − MAE(connectome) | not in this version |  |  |

<small>*Provenance: as Table 8a; snapshot 2026-09-20; seeds 1 of 5; evaluated 2026-09-20T22:07:56+00:00. BII rows as Table 8b.*</small>

Wiring sentence (rule 22, one of three pre-written strings): **not yet sayable (1 of 5 seeds)**.

Pilot rule (Section 3.9): seeds with both the fly and the shuffle finished on DTI = {1}; s_seed of the shuffle-minus-fly difference = not in this version points; verdict: **NOT IN THIS VERSION (needs connectome and shuffle on ≥ 2 seeds)**.

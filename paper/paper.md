...
<!-- STATUS: v1.1 provisional (2026-09-21). Front matter, plain-language opener, attribution, intended use, COI, footer are SETTLED
     ...
     not produced by a run. Version 1.1 = the complete pre-registered pilot (seeds 1 and 2, both tasks, all five arms) and the post-hoc examples arm (g2) for all
     four vendors (render_results.py --version 1.1); cells with no run say "not in this version"; the live spot check and the window-cap
     parity check are SKIPPED with reason; the dataset DOI is filled by splice_paper.py --dataset-doi. Later versions add seeds 3 to 5.

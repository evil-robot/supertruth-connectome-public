#!/bin/zsh
# Seeds 3 to 5 of the pre-registered five (PROTOCOL.md 8.3), same arms, same driver logic as run_pilot.sh: seeds {3,4,5} x arms {connectome, shuffle, er, mlp, ridge} x tasks {dti, bii}.
# Sequential. Restartable: a run with metrics.json is skipped.
#   nohup caffeinate -i -s zsh scripts/run_seeds3to5.sh > runs/pilot2.log 2>&1 &
set -u
cd "$(dirname "$0")/.."
PY=.venv/bin/python
STATUS=runs/pilot2_status.json
T0=$(date +%s)
SEEDS=(3 4 5); TASKS=(dti bii); ARMS=(connectome shuffle er mlp ridge)
